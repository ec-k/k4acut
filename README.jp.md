...
```bash
k4acut <入力パス> <出力パス> <開始時間> <終了時間> [--write-speed <倍率>]
```
...
- `--write-speed`: （省略可）リアルタイムに対する最大書き込み速度の倍率（例: `x1.0`, `x0.5`）．デフォルトは `x2.0`．
...
- **スロットリング**: フレーム落ちを防ぐためディスク書き込み速度を制限する．デフォルトはリアルタイムの x2.0；`--write-speed` で変更可能．
